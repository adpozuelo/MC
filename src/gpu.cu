...
// GPU (device code) interaction type (id) of a particle (see util.cpp for CPU code).
...
// GPU (device code) dot/scalar product => distance (power 2) (see util.cpp for CPU code).
...
__global__ void chpotKernel(float *r, float *chpot, int natoms, int nsp, int *itp, float *runit, float *rc2, float *al, float *bl, float *bl2, float *cl, int *nspps, int *keyp, int sp, unsigned int seed, int chpotit, float kt) {
	...
	__shared__ float deltae[NTHREAD]; // energy changes related to the excess chemical potential (block vector)
	float xyz[NDIM], rdd[NDIM];
	float rd2, eng;
	int iti, itj, nit;
	
	if (idx < chpotit) { // if global thread index is a valid chemical potential iteration; thus, every thread inserts one particle

		for (int i = 0; i < NDIM; ++i) {
			xyz[i] = curand_uniform(&state); // each global thread index generate its own particle's position from CuRand uniform random number
		}

		iti = sp; // the id for the inserted particle
		eng = 0.0; // energy accumulator set to zero
		
		for (int n = 0; n < natoms; ++n) { // for every particle in the configuration

			itj = __getIatype__(nsp, nspps, n); // the id for the configuration particle
			nit = itp[iti * nsp + itj]; // get interaction potential id between particles
			
			for (int k = 0; k < NDIM; ++k) {
				rdd[k] = xyz[k] - r[n * NDIM + k]; // calculate distante between particles (inserted and existing)
			}
			
			rd2 = __dist2__(rdd, runit); // distance power 2 between particles (inserted and existing)
			if (rd2 < rc2[nit]) { // if distance is less than interaction pontetial cutoff radio
				eng += __fpot__(rd2, nit, keyp, al, bl, cl, bl2); // accumulate energy between particles
			}
		}
		deltae[tx] = __expf(-eng / kt); // energy changes related to the excess chemical potential
		
	} else { // if global thread index is not a valid chemical potential iteration
		deltae[tx] = 0.0; // energy changes related to the excess chemical potential set to zero
	}
	...
	for (unsigned int s = blockDim.x / 2; s > 0; s >>= 1) {
		if (tx < s) { // for every thread in the lower middle half of the block
			deltae[tx] += deltae[tx + s]; // sum energy changes of its block index and its block index plus half the size of the block
		}
		__syncthreads(); // sync block threads
	}
	
	if (tx == 0) { // if block thread index is zero (the first thread of the block)
		atomicAdd(chpot, deltae[0]); // atomic (transactional) sum block energy changes to energy accumulator
	}
}

// GPU delta energy (MoveVolume algorithm) CUDA kernel.
__global__ void deKernel(int ntest, float *r, float *eng0, float *eng1, int natoms, float *rp, int nsp, int *itp, float *runit, float *rc2, float *al, float *bl, float *bl2, float *cl, int *nspps, int *keyp) {

	__shared__ float e0[NTHREAD], e1[NTHREAD]; // energy before and after particle movement (block vectors)
	...
	if (j < natoms) { // if global thread index is a valid atom/particle
		if (j == ntest) { // if global thread index is the particle to be moved (is the same particle)
			// set energies to zero
			e0[tx] = 0.0;
			e1[tx] = 0.0;
		}
		if (j != ntest) { // if global thread index is not the particle to be moved (is not the same particle)
			for (int k = 0; k < NDIM; ++k) {
			...
			if (rd2 < rc2[nit]) { // if distance is less than interaction pontetial cutoff radio
				e0[tx] = __fpot__(rd2, nit, keyp, al, bl, cl, bl2); // get energy between particles
			} else { // if distance is more than interaction potential particles don't interact
	...
	for (unsigned int s = blockDim.x / 2; s > 0; s >>= 1) { 
		if (tx < s) { // for every thread in the lower middle half of the block
			e0[tx] += e0[tx + s]; // sum energies of its block index and its block index plus half the size of the block
			e1[tx] += e1[tx + s];
		}
		__syncthreads(); // sync block threads
	}
...
*/
__global__ void __binaryReduction__(int nitems, float *g_idata, float *g_odata) {
	...
	for (unsigned int s = blockDim.x / 2; s > 0; s >>= 1) {
		if (tid < s) {  // for every thread in the lower middle half of the block
			sdata[tid] += sdata[tid + s]; // sum elements of its block index and its block index plus half the size of the block
		}
		__syncthreads(); // sync threads
	}

	if (tid == 0) { // if block thread index is zero (the first thread of the block)
		// atomic (transactional) sum block energy to energy accumulator
...
__global__ void eKernel(float *r, float *eng, int natoms, int nsp, int *itp, float *runit, float *rc2, float *al, float *bl, float *bl2, float *cl, int *nspps, int *keyp) {
	...
	for (int m = 0; m <= nbl; ++m) { // for every block
		int ml = m/nbl;
		int lim = (1 - ml) * NTHREAD + ml * rest; // calculate the limit in the block fit (particle's rest is considered)
		...
		if (mtt <= ndmol) { // if particle is valid
		 ...
		 // sync block threads (all threads/particles stores its position in current block's shared memory)
		__syncthreads();

		if (i < natoms) { // if particle (thread) is valid
			for (int j = 0; j < lim; ++j) { // for every other particle inside the block fit (check limit!)
				int jmth = j + mth; // particle global index
				if (i != jmth) { // if particle (thread) is different of particle which calculate interaction energy
					...
					rd2 = __dist2__(rdd, runit); // distance power 2 between particles
					if (rd2 < rc2[nit]) { // if distance is less than interaction pontetial cutoff radio
						energ += __fpot__(rd2, nit, keyp, al, bl, cl, bl2); // accumulate energy between particles
					}
				}
			}
		}
...
extern "C" void gpu(const int mode, const int natoms, int **itp, double *r, double *runit, double *rc2, const int nsp, int *nspps, int *keyp, double *al, double *bl, double *cl, double *bl2, const int nitmax, const int cudadevice, int *ntrial, VSLStreamStatePtr *stream, double *rdmax, const double kt, double *esr, int *naccept, const int chpotit, double *v0, double *side, double *a, double *b, double *c, const double vdmax, const char *scaling, const double pres, int *nvaccept) {
	...
	if (mode == 0) { // Initialize GPU memory.
		...
		if (rf == NULL || itp_serialized == NULL || runitf == NULL || rc2f == NULL || alf == NULL || blf == NULL || bl2f == NULL || clf == NULL) {
			fputs(errorNEM, stderr);
			exit(1);
		}
		for (int i = 0; i < natoms * NDIM; ++i) {
			rf[i] = (float)r[i];  // convert particles positions from double precision to single precision
		}
		...
		for (int i = 0; i < nsp; ++i) {
			for (int j = 0; j < nsp; ++j) {
				itp_serialized[i * nsp + j] = itp[i][j]; // interaction potentials between species must be serialized to take advantage of memory coalescenc
			}
		}
		// allocate (in GPU) interaction potentials between species from CPU to GPU
		cudaMalloc((void**)&itpdev, nsp * nsp * sizeof(int));
		cudaMemcpy(itpdev, itp_serialized, nsp * nsp * sizeof(int), cudaMemcpyHostToDevice);
		// allocate (in GPU) accumulated number of species per specie from CPU to GPU
		cudaMalloc((void**)&nsppsdev, nsp * sizeof(int));
		cudaMemcpy(nsppsdev, nspps, nsp * sizeof(int), cudaMemcpyHostToDevice);
		for (int i = 0; i < NDIM; ++i) {
			runitf[i] = (float)runit[i];  // convert normalization units of the simulation box's length sides from double precision to single precision
		}
		// allocate (in GPU) normalization units of the simulation box's length sides from CPU to GPU
		...
		float *edev;
		cudaMalloc((void**)&edev, natoms * sizeof(float)); // allocate (in GPU every thread interaction energy)
		float *engdev;
		cudaMalloc((void**)&engdev, sizeof(float)); // allocate (in GPU) the energy accumulator

		int nblock = natoms / NTHREAD; // calculate the number of blocks in GPU grid
		if (natoms % NTHREAD != 0) {
			++nblock;
		}
		
		// call CUDA energy kernel
		eKernel<<<nblock, NTHREAD>>>(rdev, edev, natoms, nsp, itpdev, runitdev, rc2dev, aldev, bldev, bl2dev, cldev, nsppsdev, keypdev);
		//cudaCheckError();
		cudaMemset(engdev, 0, sizeof(float)); // set the energy's accumulator to zero before reduction

        // call CUDA binary reduction kernel
		__binaryReduction__<<<nblock, NTHREAD>>>(natoms, edev, engdev);
		//cudaCheckError();
		cudaMemcpy(&eng, engdev, sizeof(float), cudaMemcpyDeviceToHost); // copy energy accumulator from GPU to CPU
		...
		*esr = eng / 2; // return energy
	...
	} else if (mode == 2) { // Move atoms Metropolis Montecarlo algorithm in parallel mode.
		...
		float *e0dev, *e1dev;
		// allocate (in GPU) energy accumulators (after and before the particle's movement)
		cudaMalloc((void**)&e0dev, sizeof(float));
		cudaMalloc((void**)&e1dev, sizeof(float));
		
		const int nblock = (natoms + (NTHREAD - 1)) / NTHREAD; // calculate the number of blocks in GPU grid
		
		for (int i = 0; i < natoms; ++i) { // for every atom/particle

			(*ntrial)++; // update trial counter
			vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD, *stream, h_size, harvest, 0, 1); // generate uniform random numbers
			...
			for (int j = 0; j < NDIM; ++j) {
				...
				rpf[j] = (float)rp[j]; // convert particle's positions (after the movement) from double precision to single precision
			}
			...
			deKernel<<<nblock, NTHREAD>>>(ntest, rdev, e0dev, e1dev, natoms, rpdev, nsp, itpdev, runitdev, rc2dev, aldev, bldev, bl2dev, cldev, nsppsdev, keypdev);
			//cudaCheckError();
			// copy energy accumulators (energy after and before the movement) from GPU to CPU
			...
			if (deltae < 0.0) {  // if diference is less than zero
				...
				// accept the movement inside GPU (avoid recurrent and duplicated particle positions copy between CPU and GPU)
				cudaMemcpy(rdev + ntest * NDIM, rpf, NDIM * sizeof(float), cudaMemcpyHostToDevice); // only copy the position of the moved particle
				*esr += deltae; // update current iteration/step energy
				(*naccept)++; // update number of success movements of particles in move atoms algorithm
				
			} else { // else, movement not accepted by energy's difference
				double xi[1];
				vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD, *stream, 1, xi, 0, 1); // generate uniform random number
				if (exp(-deltae / kt) > xi[0]) { // if random number is less than Boltzmann entropy
					for (int k = 0; k < NDIM; ++k) {
						r[ntest * NDIM + k] = rp[k];  // accept the movement
					}
					// accept the movement inside GPU (avoid recurrent and duplicated particle positions copy between CPU and GPU)
		...
		const int nblock = (chpotit + (NTHREAD - 1)) / NTHREAD; // calculate the number of blocks in GPU grid
		...
		for (int i = 0; i < nsp; ++i) { // for every specie
			...
			cudaMemset(edev, 0, sizeof(float)); // set excess chemical potential to zero

            //chpotKernel<<<nblock, NTHREAD>>>(rdev, edev, natoms, nsp, itpdev, runitdev, rc2dev, aldev, bldev, bl2dev, cldev, nsppsdev, keypdev, i, time(NULL), chpotit, (float)kt); // random mode
			chpotKernel<<<nblock, NTHREAD>>>(rdev, edev, natoms, nsp, itpdev, runitdev, rc2dev, aldev, bldev, bl2dev, cldev, nsppsdev, keypdev, i, 1 + counter, chpotit, (float)kt); // deterministic mode
			//cudaCheckError();
		...
		float *runitf = (float*)malloc(NDIM * sizeof(float));
		if (sideOld == NULL || aOld == NULL || bOld == NULL || cOld == NULL || runitOld == NULL || runitf == NULL) {
			fputs(errorNEM, stderr);
			exit(1);
		}
		...
		if (strcmp(scaling, "ortho") == 0) { // is scaling is ortho
			double yzi[2];
			vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD, *stream, 2, yzi, 0, 1); // generate uniform random number
			side[1] += (2 * yzi[0] - 1) * vdmax; // move Y length side using random number
			side[2] += (2 * yzi[1] - 1) * vdmax; // move Z length side using random number
		...
		*v0 = side[0] * side[1] * side[2]; // calculate new simulation box volume
		...
		for (int i = 0; i < NDIM; ++i) {
			runit[i] = side[i]; // set new normalization units of the simulation box's length sides
		}
		...
		float eng;
		float *edev;
		cudaMalloc((void**)&edev, natoms * sizeof(float)); // allocate (in GPU) the particle's interaction energy vector
		float *engdev;
		cudaMalloc((void**)&engdev, sizeof(float)); // allocate (in GPU) the energy accumulator
		...
		eKernel<<<nblock, NTHREAD>>>(rdev, edev, natoms, nsp, itpdev, runitdev, rc2dev, aldev, bldev, bl2dev, cldev, nsppsdev, keypdev);
		//cudaCheckError();
		cudaMemset(engdev, 0, sizeof(float)); // set the energy's accumulator to zero
		...
		*esr = eng / 2; // return energy
		...
		double deltaEsr = *esr - esrOld; // get difference of energies (after and before volume's movement)
		double cond = exp(-(deltaEsr + pres * (*v0 - v0Old)) / kt + (double)natoms * log(*v0 / v0Old)); // acceptance criteria for volume changes
		
		vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD, *stream, 1, xi, 0, 1); // generate uniform random number
		
		if (cond > xi[0]) { // if random number is less than acceptance criteria for volume changes
			...
			*v0 = v0Old;
			...
			for (int i = 0; i < NDIM; ++i) {
				runitf[i] = (float)runitOld[i];  // convert normalization units of the simulation box's length sides from double precision to single precision
			}
			// allocate (in GPU) normalization units of the simulation box's length sides from CPU to GPU
			cudaMemcpy(runitdev, runitf, NDIM * sizeof(float), cudaMemcpyHostToDevice);
		}
