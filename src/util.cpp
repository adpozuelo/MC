...
void initLattice(const double rho, double **a, double **b, double **c,
  ...
  *c = (double *)calloc(NDIM, sizeof(double));
  ...
  double boxl = pow((double)natoms / rho,
                    0.33333333333);  // box length depends of the density
  (*a)[0] = (*b)[1] = (*c)[2] =
      boxl /
      sigma_o;  // normalize box length in reference of potential parameters
  ...
  *r = (double *)malloc(natoms * NDIM * sizeof(double));
  ...
  int mside = (int)ceil(
      pow((double)natoms / 4.0, 0.333333));  // number of FCC individual cells
                                             // (4 individual nodes per cell)
  ...
  int *random1 =
      (int *)malloc(natoms * sizeof(int));  // random positions particle A
  int *random2 =
      (int *)malloc(natoms * sizeof(int));  // random positions particle B
  ...
  for (int p = 0; p < riterations; ++p) {
    ...
    for (int i = 0; i < natoms; ++i) {  // for natoms iterations
      xtmp = (*r)[random1[i] *
                  NDIM];  // particle A position go to tmp particle position
      ytmp = (*r)[random1[i] * NDIM + 1];
      ztmp = (*r)[random1[i] * NDIM + 2];
      (*r)[random1[i] * NDIM] =
          (*r)[random2[i] *
               NDIM];  // particle B position go to particle A position
      (*r)[random1[i] * NDIM + 1] = (*r)[random2[i] * NDIM + 1];
      (*r)[random1[i] * NDIM + 2] = (*r)[random2[i] * NDIM + 2];
      (*r)[random2[i] * NDIM] =
          xtmp;  // tmp particle (initial particle A) go to particle B position
