 ...
 *
 * At present simulates bulk systems composed of soft spherical particles without charges.
 * Implemented ensembles: NVT and NPT.
 * Chemical potential implemented only for NVT ensemble.
 *
 ...
 *          CONFIG     : initial configuration in DLPOLY 2 format (to be generalized).
 *
 * Output files:
 ...
 *           conf.xyz        : initial and trajectory configuration in VMD format.
 *
 * Program units:
...
int main(int argc, char *argv[]) {
	...
	if (argc < 2 || argc > 3) {	// Input arguments control
	 	puts("Usage: mc.exe mode [cudaDevice]\n\nmode:\nserial -> CPU\ngpu -> NVIDIA GPU\n\n[cudaDevice]:\nInteger from 0 to 7 (default = 0)\n");
	 	exit(1);
	 }
	...
	if (strcmp(argv[1], "gpu") == 0) { // Get cuda device from input arguments
		if (argv[2] != NULL) {
			...
			if (cudadevice < 0 || cudadevice > 7) {
				printf("ERROR: '%d' is not a valid cudaDevice integer (from 0 to 7 gpus supported)\n!", cudadevice);
				puts("Usage: mc.exe mode [cudaDevice]\n\nmode:\nserial -> CPU\ngpu -> NVIDIA GPU\n\n[cudaDevice]:\nInteger from 0 to 7 (default = 0)\n");
				exit(1);
			}
		}
	...
	bool firstRun = true; // First run boolean to control if equilibrium phase is over
	/**
	 ...
	 */
	int nsp, nitmax, natoms, naccept, nvaccept, ntrial, naver, nstep, nequil, nb, wc, chpotnb, chpotit;
	natoms = naccept = nvaccept = ntrial = naver = 0; // Accumulators initialized to zero
	/**
	 ...
	 */
	double *cl, *bl2, *al, *bl, *rc2, *a, *b, *c, *side, *runit, *rdmax, *r, *sideav;
	/**
	...
	readSystemDatFile(&initcf, &nsp, &nitmax, &atoms, &nspps, &natoms, &units, &keyp, &cl, &bl2, &al, &bl, &rc2, &rc, &itp, &rho, &sigma_o, &eps_o);

	if (strcmp(initcf, "dlp") == 0) { // If configuration mode is dlp read it from CONFIG input file
		readConfigFile(&a, &b, &c, nsp, natoms, &ntype, &r, nspps, atoms, &side, &runit, &v0, sigma_o);
	} else if (strcmp(initcf, "lattice") == 0) { // If configuration mode is lattice create a lattice configuration from scratch
		initLattice(rho, &a, &b, &c, nsp, natoms, &ntype, &r, nspps, atoms, &side, &runit, &v0, &stream, sigma_o);
	} else {
	...
	if (strcmp(runMode, "serial") == 0) {
		esr = energycpu(natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2);
	}
	if (strcmp(runMode, "gpu") == 0) {
		// Initialize GPU (mode 0)
		gpu(0, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2, nitmax, cudadevice, &ntrial, &stream, rdmax, kt, &esr, &naccept, chpotit, &v0, side, a, b, c, vdmax, scaling, pres, &nvaccept);
		// Energy GPU (mode 1)
		gpu(1, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2, nitmax, cudadevice, &ntrial, &stream, rdmax, kt, &esr, &naccept, chpotit, &v0, side, a, b, c, vdmax, scaling, pres, &nvaccept);
	}
	...
	if (strcmp(ensemble, "nvt") == 0) {
		...
		puts(" No. moves  % accept.    % acc. vol.    E_tot           E_sr      Vol");
		puts("--------------------------------------------------------------------------------");
	}
	...
	for (int istep = 1; istep <= nstep; ++istep) { // For every simulation step
		...
		if (strcmp(runMode, "serial") == 0) {
			moveAtoms(&ntrial, natoms, &stream, rdmax, runit, r, nsp, nspps, itp, rc2, keyp, al, bl, cl, bl2, kt, &esr, &naccept);
		}
		if (strcmp(runMode, "gpu") == 0) {
			// MoveAtoms GPU (mode 2)
			gpu(2, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2, nitmax, cudadevice, &ntrial, &stream, rdmax, kt, &esr, &naccept, chpotit, &v0, side, a, b, c, vdmax, scaling, pres, &nvaccept);
		}
		clock_t mAtomsEnd = clock(); // Move atoms control time end
		moveAtomsTime += (double) (mAtomsEnd - mAtomsBegin) / CLOCKS_PER_SEC; // Time for move atoms algorithm
		...
		if (strcmp(ensemble, "npt") == 0) { // If simulation ensemble is npt
			...
			if (strcmp(runMode, "serial") == 0) {
				moveVolume(&esr, &v0, side, a, b, c, runit, &stream, vdmax, scaling, pres, kt, natoms, &nvaccept, itp, r, rc2, nsp, nspps, keyp, al, bl, cl, bl2);
			}
			if (strcmp(runMode, "gpu") == 0) {
				// MoveVolume GPU (mode 4)
				gpu(4, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2, nitmax, cudadevice, &ntrial, &stream, rdmax, kt, &esr, &naccept, chpotit, &v0, side, a, b, c, vdmax, scaling, pres, &nvaccept);
			}
			clock_t mVolumeEnd = clock(); // Move volume control time end
			moveVolumeTime += (double) (mVolumeEnd - mVolumeBegin) / CLOCKS_PER_SEC; // Time for move volume algorithm
		}
			
		if (istep >= nequil) { // If equilibrium phase is over
			histograms(ensemble, &etotal, ehisto, natoms, esr, v0, eref, deltaeng, deltar, rhisto); // Generate histograms

			if (strcmp(ensemble, "nvt") == 0) { // If simulation ensemble is nvt
				if (chpotnb != 0 && istep % chpotnb == 0) { // If chemical potential is enabled and it must be calculated
					clock_t chPotentialBegin = clock(); // Chemical potential time begin
					// Chemical potential algorithm in selected run mode
					if (strcmp(runMode, "serial") == 0) {
						chpotentialcpu(chpotit, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2, kt);
					}
					if (strcmp(runMode, "gpu") == 0) {
						// Chemical Potential GPU (mode 3)
						gpu(3, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2, nitmax, cudadevice, &ntrial, &stream, rdmax, kt, &esr, &naccept, chpotit, &v0, side, a, b, c, vdmax, scaling, pres, &nvaccept);
					}
					clock_t chPotentialEnd = clock(); // Chemical potential time end
					chPotentialTime += (double) (chPotentialEnd - chPotentialBegin) / CLOCKS_PER_SEC; // Time for chemical potential algorithm
				}
			}
		}
		...
		// If configuration file output's write is enabled and it must be writed (only in serial mode!)
		if (wc != 0 && istep % wc == 0 && strcmp(runMode, "serial") == 0) {
			writeConf(natoms, nspps, atoms, r, runit); // Write configuration to conf.xyz output file
		}
	}
	...
	if (strcmp(runMode, "gpu") == 0) {
		...
		gpu(1, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2, nitmax, cudadevice, &ntrial, &stream, rdmax, kt, &esr, &naccept, chpotit, &v0, side, a, b, c, vdmax, scaling, pres, &nvaccept);
		// Release GPU (mode 5)
		gpu(5, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2, nitmax, cudadevice, &ntrial, &stream, rdmax, kt, &esr, &naccept, chpotit, &v0, side, a, b, c, vdmax, scaling, pres, &nvaccept);
	}
	...
	double time_spent = (double) (end - begin) / CLOCKS_PER_SEC; // Simulation total time
