...
int main(int argc, char *argv[]) {
  ...
  // Get run mode (serial or parallel) from input arguments
  char *runMode;
  ...
  // Default cuda device
  int cudadevice = 0;
  // Get cuda device from input arguments
  ...
  /**
     ...
     every specie)
   */
  int nsp, nitmax, natoms, naver, nstep, nequil, nb, wc, chpotnb, chpotit;
  unsigned long int naccept, nvaccept;
  // Accumulators initialized to zero
  natoms = naccept = nvaccept = naver = 0;
  /**
  ...
  readSystemDatFile(&initcf, &nsp, &nitmax, &atoms, &nspps, &natoms, &units,
                    ...
                    &eps_o);
  // If configuration mode is dlp read it from CONFIG input file
  if (strcmp(initcf, "dlp") == 0) {
    readConfigFile(&a, &b, &c, nsp, natoms, &ntype, &r, nspps, atoms, &side,
                   &runit, &v0, sigma_o);
    // If configuration mode is lattice create a configuration from scratch
  ...
  if (strcmp(runMode, "gpu") == 0) {
    ...
    gpu(0, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2,
        nitmax, cudadevice, &stream, rdmax, kt, &esr, &naccept, chpotit, &v0,
        side, a, b, c, vdmax, scaling, pres, &nvaccept);
    // Energy GPU (mode 1)
  ...
  for (int ntrial = 1; ntrial <= nstep; ++ntrial) {
    clock_t mAtomsBegin = clock();  // Move atoms control time begin
    // Move atoms algorithm in selected run mode
    if (strcmp(runMode, "serial") == 0) {
      moveAtoms(natoms, &stream, rdmax, runit, r, nsp, nspps, itp, rc2, keyp,
                al, bl, cl, bl2, kt, &esr, &naccept);
    }
    if (strcmp(runMode, "gpu") == 0) {
    ...
    if (strcmp(ensemble, "npt") == 0) {  // If simulation ensemble is npt
      ...
      if (strcmp(runMode, "gpu") == 0) {
        ...
        gpu(4, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl, bl2,
            nitmax, cudadevice, &stream, rdmax, kt, &esr, &naccept, chpotit,
            &v0, side, a, b, c, vdmax, scaling, pres, &nvaccept);
      }
      // Move volume control time end
      clock_t mVolumeEnd = clock();
      // Time for move volume algorithm
      moveVolumeTime += (double)(mVolumeEnd - mVolumeBegin) / CLOCKS_PER_SEC;
    }
    if (ntrial >= nequil) {  // If equilibrium phase is over
      // Generate histograms
      histograms(ensemble, &etotal, ehisto, natoms, esr, v0, eref, deltaeng,
                 deltar, rhisto);
      if (strcmp(ensemble, "nvt") == 0) {  // If simulation ensemble is nvt
        // If chemical potential is enabled and it must be calculated
        if (chpotnb != 0 && ntrial % chpotnb == 0) {
          ...
          if (strcmp(runMode, "gpu") == 0) {
            ...
            gpu(3, natoms, itp, r, runit, rc2, nsp, nspps, keyp, al, bl, cl,
                bl2, nitmax, cudadevice, &stream, rdmax, kt, &esr, &naccept,
                chpotit, &v0, side, a, b, c, vdmax, scaling, pres, &nvaccept);
          }
          // Chemical potential time end
          clock_t chPotentialEnd = clock();
          // Time for chemical potential algorithm
          chPotentialTime +=
              (double)(chPotentialEnd - chPotentialBegin) / CLOCKS_PER_SEC;
        }
      }
    }
    if (ntrial % nb == 0) {  // If statistics must be calculated
      if (ntrial > nequil) {  // If equilibrium phase is over
    ...
    // (only in serial mode!)
    if (wc != 0 && ntrial % wc == 0 && strcmp(runMode, "serial") == 0) {
