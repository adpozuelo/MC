...
double fpot(const double r2, const int nit, int *keyp, double *al, double *bl, double *cl, double *bl2) {
	...
	if (keyp[nit] == 1) { // Morse potential (https://en.wikipedia.org/wiki/Morse_potential)
		rr = sqrt(r2);
		double expp = exp(-bl[nit] * (rr - cl[nit]));
		return al[nit] * ((1 - expp) * (1 - expp) - 1);

	} else if (keyp[nit] == 2) { // Lennard Jones potential (https://en.wikipedia.org/wiki/Lennard-Jones_potential)
	...
	for (int i = 0; i < natoms; ++i) { // for every particle
		for (int j = i + 1; j < natoms; ++j) { // travel the other particles
			...
			for (int k = 0; k < NDIM; ++k) {
				rdd[k] = r[i * NDIM + k] - r[j * NDIM + k];  // calculate distante between particles
			}
			...
			if (rd2 < rc2[nit]) { // if distance is less than interaction pontetial cutoff radio
				eng += fpot(rd2, nit, keyp, al, bl, cl, bl2); // accumulate energy between particles
			}
		}
	}
...
void chpotentialcpu(const int chpotit, const int natoms, int **itp, double *r, double *runit, double *rc2, const int nsp, int *nspps, int *keyp, double *al, double *bl, double *cl, double *bl2, const double kt) {
	...
	for (int i = 0; i < nsp; ++i) { // for every specie
		...
		for (int j = 0; j < chpotit; ++j) { // chemical potential iterations 
			...
			for (int k = 0; k < NDIM; ++k) {
				xyz[k] = harvest[j * NDIM + k]; // get random inserted particle's position
			}
			
			eng = 0.0; // energy accumulator set to zero
			for (int n = 0; n < natoms; ++n) { // for every particle in the configuration

				itj = getIatype(nsp, nspps, n);  // the id for the configuration particle
				...
				rd2 = dist2(rdd, runit);  // distance power 2 between particles (inserted and existing)
				if (rd2 < rc2[nit]) {  // if distance is less than interaction pontetial cutoff radio
					eng += fpot(rd2, nit, keyp, al, bl, cl, bl2); // accumulate energy between particles
				}
			}
			deltae[j] = exp(-eng / kt); // energy changes related to the excess chemical potential
		}
