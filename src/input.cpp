...
void readSystemDatFile(char **initcf, int *nsp, int *nitmax, char ***atoms, int **nspps, int *natoms, char **units, int **keyp, double **cl, double **bl2, double **al, double **bl, double **rc2, double ***rc, int ***itp, double *rho, double *sigma_o, double *eps_o) {
	...
	*initcf = (char*) malloc(strlen(buffer) * sizeof(char));
	...
	} else if (strcmp(*initcf, "lattice") == 0) { // if configuration model is lattice
		if (sscanf(line, "%d %lf", nsp, rho) < 1) { // read both the configuration model and the simulation box density
			fputs(errorIF, stderr);
			exit(1);
		}
	} else {
	...
	*nitmax = (*nsp * *nsp + *nsp) / 2; // calculate the max number of interactions between species
	...
	*atoms = (char**) malloc(*nsp * sizeof(char*));
	...
	for (int i = 0; i < *nsp; ++i) { // for every specie
		readLine(systemDatFile, filename, line);
		// read specie's chemical character representation and the number of particles per specie
	...
	*itp = (int**) malloc(*nsp * sizeof(int*));
	if (aa == NULL || bb == NULL || cc == NULL || *keyp == NULL || *cl == NULL || *bl2 == NULL || *al == NULL || *bl == NULL || *rc2 == NULL || *rc == NULL || *itp == NULL) {
		fputs(errorNEM, stderr);
		exit(1);
	}
	for (int i = 0; i < *nsp; ++i) {
		...
		if (aa[i] == NULL || bb[i] == NULL || cc[i] == NULL || (*rc)[i] == NULL	|| (*itp)[i] == NULL) {
			fputs(errorNEM, stderr);
			exit(1);
		}
	}
	...
	for (int i = 0; i < *nsp; ++i) {
		for (int j = i; j < *nsp; ++j) { // for every interaction between species
			...
			if (strcmp(buffer, "mors") == 0) { // if potential is Morse
				...
				readLine(systemDatFile, filename, line);
				if (sscanf(line, "%lf %lf %lf %lf", &aa[i][j], &bb[i][j], &cc[i][j], &(*rc)[i][j]) < 4) { // read Morse parameters
					fputs(errorIF, stderr);
					exit(1);
				}
				...
				readLine(systemDatFile, filename, line);
				if (sscanf(line, "%lf %lf %lf", &aa[i][j], &bb[i][j], &(*rc)[i][j]) < 3) { // read Morse parameters
					fputs(errorIF, stderr);
					exit(1);
				}
			} else {
	...
	// normalize interaction potential parameters for optimize floating point representation errors
	if ((*keyp)[0] == 1) {
		*sigma_o = (*cl)[0];
...
void readConfigFile(double **a, double **b, double **c, const int nsp, const int natoms, int **ntype, double **r, int *nspps, char **atoms, double **side, double **runit, double *v0, const double sigma_o) {
	...
	const char *errorInvalidFormatConfigFile = "ERROR: Invalid '../data/CONFIG' file format!\n";
	...
	readLine(configFile, filename, line);
	if (sscanf(line, "%d", &keytrj) < 1) {  // read trajectories description dimensions
	...
	*c = (double*) calloc(NDIM, sizeof(double));
	...
	for (int i = 0; i < NDIM; ++i) {
		readLine(configFile, filename, line);
		if (sscanf(line, "%lf %lf %lf", &(*a)[i], &(*b)[i], &(*c)[i]) < 3) { // read the simulations's box dimensions (XYZ)
			fputs(errorInvalidFormatConfigFile, stderr);
			exit(1);
		}
	}

	for (int i = 0; i < NDIM; ++i) { // normalize the simulation's box dimensions in reference to potential parameters
		(*a)[i] /= sigma_o;
		(*b)[i] /= sigma_o;
		(*c)[i] /= sigma_o;
	}
	...
	if ((*a)[1] * (*a)[1] + (*a)[2] * (*a)[2] + (*b)[0] * (*b)[0] + (*b)[2] * (*b)[2] + (*c)[0] * (*c)[0] + (*c)[1] * (*c)[1] > 1.0e-7) {
	...
	*r = (double*) malloc((natoms) * NDIM * sizeof(double));
	// particles positions (xyz) by specie used by short particles while they are inserted
	...
	for (int i = 0; i < natoms; ++i) { // for every particle
		readLine(configFile, filename, line);
		if (sscanf(line, "%s %d", buffer, &iatm) < 2) { // read chemical character representation and the id of the particle
			fputs(errorInvalidFormatConfigFile, stderr);
			exit(1);
		}
		...
		for (int j = 0; j < nsp; ++j) { // for every specie
			if (strcmp(buffer, atoms[j]) == 0) { // select which specie's vector must be selected

				readLine(configFile, filename, line);
				// insertion short particle in species vector
				if (sscanf(line, "%lf %lf %lf", &rnsp[j][(*ntype)[j] * NDIM], &rnsp[j][(*ntype)[j] * NDIM + 1], &rnsp[j][(*ntype)[j] * NDIM + 2]) < 3) {
					fputs(errorInvalidFormatConfigFile, stderr);
					exit(1);
				}
	...
	for (int i = 1; i < nsp; ++i) {
		for (int j = 0; j < i; ++j) {
			nspps[i] += nspps[j]; // create the accumulated number of species per specie
		}
	}
	...
	for (int i = 0; i < nsp; ++i) {
		for (int j = 0; j < (*ntype)[i] * NDIM; ++j) {
			// linearize and normalize the shorted by specie particle's positions (XYZ) in reference to potential parameters
	...
	*runit = (double*) malloc(NDIM * sizeof(double));
	...
	(*runit)[0] = sqrt(rdd); // normalization units of the simulation box's length X side
	rdd = 0.0;
	for (int i = 0; i < NDIM; ++i) {
		rd[i] = (*b)[i] * (*b)[i];
		rdd += rd[i];
	}
	(*runit)[1] = sqrt(rdd); // normalization units of the simulation box's length Y side
	rdd = 0.0;
	for (int i = 0; i < NDIM; ++i) {
		rd[i] = (*c)[i] * (*c)[i];
		rdd += rd[i];
	}
	(*runit)[2] = sqrt(rdd); // normalization units of the simulation box's length Z side
	free(rd);
	...
	*v0 = (*a)[0] * (*b)[1] * (*c)[2] + (*a)[1] * (*b)[2] * (*c)[0]	+ (*a)[2] * (*b)[0] * (*c)[1] - (*a)[2] * (*b)[1] * (*c)[0]	- (*a)[1] * (*b)[0] * (*c)[2] - (*a)[0] * (*b)[2] * (*c)[1];

	for (int i = 0; i < natoms; ++i) {
		for (int j = 0; j < NDIM; ++j) {
			// normalize particle's positions (XYZ) in reference to units of the simulation box's length (XYZ) side
			(*r)[i * NDIM + j] /= (*runit)[j];
		}
	}
}
...
void readRunMCFile(char **ensemble, int *nstep, int *nequil, int *nb, int *wc, double **rdmax, double *temp, double *deltaeng, unsigned long long int **ehisto, double *vdmax, char **scaling, double *pres, double *deltar, double **sideav, unsigned long long int **rhisto, const double eps_o, const double sigma_o, int *chpotnb, int *chpotit) {
	...
	const char *errorInvalidFormatRunFile =	"ERROR: Invalid '../data/runMC.dat' file format!\n";
	...
	*ensemble = (char*) malloc(strlen(buffer) * sizeof(char));
	...
	readLine(runFile, filename, line);
	// read number of total simulation's steps, number of equilibrium phase steps, every steps averages will be executed and every steps configuration file will be writed variables
	...
	if (strcmp(*ensemble, "nvt") == 0) { // if ensemble is nvt
		readLine(runFile, filename, line);
		// read every steps chemical potential will be executed and chemical potential iterations (number of particles inserted for every specie)
	...
	*rdmax = (double*) malloc(NDIM * sizeof(double));
	...
	if (strcmp(*ensemble, "nvt") == 0) { // if ensemble is nvt
		...
		readLine(runFile, filename, line);
		if (sscanf(line, "%lf", deltaeng) < 1) { // read nvt delta grid for energy histogram
			fputs(errorInvalidFormatRunFile, stderr);
			exit(1);
		}
	...
	if (strcmp(*ensemble, "npt") == 0) { // if ensemble is ntp
		...
		*scaling = (char*) malloc(strlen(buffer) * sizeof(char));
		...
		readLine(runFile, filename, line);
		if (sscanf(line, "%lf %lf", temp, pres) < 2) { // read simulation box temperature and simulation box pression
			fputs(errorInvalidFormatRunFile, stderr);
			exit(1);
		}

		readLine(runFile, filename, line);
		if (sscanf(line, "%lf", deltar) < 1) { // read npt delta grid for density histogram
			fputs(errorInvalidFormatRunFile, stderr);
			exit(1);
		}
		...
		*rhisto = (unsigned long long int*) calloc(NRMAX + 1, sizeof(unsigned long long int));
		if (*rhisto == NULL) {
			fputs(errorNEM, stderr);
			exit(1);
		}
