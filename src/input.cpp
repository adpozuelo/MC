  ...
  if (strcmp(*initcf, "dlp") == 0) {    // if configuration model is dlp
    if (sscanf(line, "%d", nsp) < 1) {  // read the number os species
      ...
      exit(1);
             ...
             0) {  // if configuration model is lattice
    if (sscanf(line, "%d %lf", nsp, rho) <
        1) {  // read both the configuration model and the simulation box
              // density
      fputs(errorIF, stderr);
      exit(1);
    }
    ...
    for (int j = i; j < *nsp; ++j) {  // for every interaction between species
      ...
      if (strcmp(buffer, "mors") == 0) {  // if potential is Morse
        ...
        if (sscanf(line, "%lf %lf %lf %lf", &aa[i][j], &bb[i][j], &cc[i][j],
                   &(*rc)[i][j]) < 4) {  // read Morse parameters
          fputs(errorIF, stderr);
          exit(1);
        }
...
void readConfigFile(double **a, double **b, double **c, const int nsp,
  ...
  readLine(configFile, filename, line);
  if (sscanf(line, "%d", &keytrj) <
      1) {  // read trajectories description dimensions
  ...
  *c = (double *)calloc(NDIM, sizeof(double));
  ...
  for (int i = 0; i < NDIM; ++i) {
    readLine(configFile, filename, line);
    if (sscanf(line, "%lf %lf %lf", &(*a)[i], &(*b)[i], &(*c)[i]) <
        3) {  // read the simulations's box dimensions (XYZ)
      fputs(errorInvalidFormatConfigFile, stderr);
      exit(1);
    }
  }

  for (int i = 0; i < NDIM; ++i) {  // normalize the simulation's box dimensions
                                    // in reference to potential parameters
    (*a)[i] /= sigma_o;
    (*b)[i] /= sigma_o;
    (*c)[i] /= sigma_o;
  }
  ...
  *r = (double *)malloc((natoms)*NDIM * sizeof(double));
  ...
  for (int i = 0; i < natoms; ++i) {  // for every particle
    readLine(configFile, filename, line);
    if (sscanf(line, "%s %d", buffer, &iatm) <
        2) {  // read chemical character representation and the id of the
              // particle
      fputs(errorInvalidFormatConfigFile, stderr);
      exit(1);
    }
    ...
    for (int j = 0; j < nsp; ++j) {  // for every specie
      if (strcmp(buffer, atoms[j]) ==
          0) {  // select which specie's vector must be selected
  ...
  *runit = (double *)malloc(NDIM * sizeof(double));
  ...
  (*runit)[0] =
      sqrt(rdd);  // normalization units of the simulation box's length X side
  rdd = 0.0;
  for (int i = 0; i < NDIM; ++i) {
    rd[i] = (*b)[i] * (*b)[i];
    rdd += rd[i];
  ...
  (*runit)[1] =
      sqrt(rdd);  // normalization units of the simulation box's length Y side
  rdd = 0.0;
  for (int i = 0; i < NDIM; ++i) {
    rd[i] = (*c)[i] * (*c)[i];
    rdd += rd[i];
  ...
  (*runit)[2] =
      sqrt(rdd);  // normalization units of the simulation box's length Z side
  free(rd);
  ...
  *rdmax = (double *)malloc(NDIM * sizeof(double));
  ...
  if (strcmp(*ensemble, "nvt") == 0) {  // if ensemble is nvt
    ...
    if (sscanf(line, "%lf %lf %lf %lf", final_sm_rate, &(*rdmax)[0],
               &(*rdmax)[1],
               &(*rdmax)[2]) < 4) {  // read maximum trial displacements
      fputs(errorInvalidFormatRunFile, stderr);
      exit(1);
    }
    ...
    readLine(runFile, filename, line);
    if (sscanf(line, "%lf", deltaeng) <
        1) {  // read nvt delta grid for energy histogram
      fputs(errorInvalidFormatRunFile, stderr);
      exit(1);
    }
  ...
  if (strcmp(*ensemble, "npt") == 0) {  // if ensemble is ntp
    ...
    *scaling = (char *)malloc(strlen(buffer) * sizeof(char));
    ...
    readLine(runFile, filename, line);
    if (sscanf(line, "%lf %lf", temp, pres) <
        2) {  // read simulation box temperature and simulation box pression
      fputs(errorInvalidFormatRunFile, stderr);
      exit(1);
    }

    readLine(runFile, filename, line);
    if (sscanf(line, "%lf", deltar) <
        1) {  // read npt delta grid for density histogram
      fputs(errorInvalidFormatRunFile, stderr);
      exit(1);
    }
