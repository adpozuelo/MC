...
void printout(const bool inst, double *etotal, double *eref, const double esr,
  ...
  double n_accept_trial = naccept / ((double)ntrial * (double)natoms);
  // DEBUG!!
  if (n_accept_trial < 0 || naccept < 0 || ntrial < 0 || natoms < 0) {
    printf(
        "\n\nn_accept_trial = %lf, naccept = %lu, ntrial = %d, natoms = %d\n\n",
        n_accept_trial, naccept, ntrial, natoms);
    exit(1);
  }
  // DEBUG!!
  double nv_accept_trial = nvaccept / (double)ntrial;
  // % of accepted atom's movements
  double nacceptper = 100 * n_accept_trial;
  // % of accepted volume's movements
  double nvacceptper = 100 * nv_accept_trial;
  *etotal = esr;
  if (inst) {  // write average information to a output file boolean control
    ...
    // if ensemble is nvt write information (not normalized!) to the output file
    if (strcmp(ensemble, "nvt") == 0) {
      ...
      // if ensemble is npt write information (not normalized!) to the output
      // file
    } else if (strcmp(ensemble, "npt") == 0) {
      if (fprintf(ioth,
                  "     %2d    %.4lf    %.4lf    %.3lf       %.3lf      %.2lf",
                  ntrial, nacceptper, nvacceptper, etav * eps_o / naver,
                  esav * eps_o / naver,
                  volav * sigma_o * sigma_o * sigma_o / naver) == EOF) {
        printf("ERROR: cannot write to '%s' file!\n", filenameTh);
        exit(1);
      }
      // write volume information (not normalized!) to the output file
    ...
    if (strcmp(ensemble, "npt") == 0) {
      *vdmax *= (nv_accept_trial / final_sm_rate);
    }
  ...
  if (strcmp(ensemble, "nvt") == 0) {
    ...
    // if ensemble is npt write information (not normalized!) to the output file
  } else if (strcmp(ensemble, "npt") == 0) {
    if (fprintf(
            iothi,
            "     %2d    %.4lf        %.4lf         %.3lf      %.3lf     %.2lf",
            ntrial, nacceptper, nvacceptper, *etotal * eps_o, esr * eps_o,
...
void initOutputFiles(const char *ensemble, const int nsp, char **atoms) {
  ...
  if (strcmp(ensemble, "nvt") == 0) {
    ...
    // if ensemble is npt write the header to output file
  } else if (strcmp(ensemble, "npt") == 0) {
...
void writeConf(const int natoms, int *nspps, char **atoms, double *r,
  ...
  for (int i = 0; i < natoms; ++i) {
    for (int j = 0; j < NDIM; ++j) {
      // not normalized particles positions (xyz)
      rr[i * NDIM + j] = r[i * NDIM + j] * runit[j];
    }
  }
  ...
  for (int i = 0; i < natoms; ++i) {
    if (i < nspps[nsppsc]) {
      // write specie's chemical character representation to output file
    ...
    for (int j = 0; j < NDIM; ++j) {
      // write particle positions (xyz) to output file
      if (fprintf(fp, "%lf ", rr[i * NDIM + j]) == EOF) {
        printf("ERROR: cannot write to '%s' file!\n", filename);
        exit(1);
      }
    }
