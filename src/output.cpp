...
void printout(const bool inst, double *etotal, double *eref, const double esr, const char *ensemble, double *sideav, const double etav, const int naver, const double v0, const double esav, const double volav, double *side, const int natoms, const int ntrial, const int naccept, const int nvaccept, const double sigma_o, const double eps_o, const double final_sm_rate, double *vdmax, double *rdmax) {
	...
	*etotal = esr;
	...
	if (inst) { // write average information to a output file boolean control
		...
		if (strcmp(ensemble, "nvt") == 0) { // if ensemble is nvt write information (not normalized!) to the output file
			if (fprintf(ioth, "     %2d    %.4lf        %.3lf       %.3lf\n", ncycles, nacceptper, etav * eps_o / naver, esav * eps_o / naver) == EOF) {
				printf("ERROR: cannot write to '%s' file!\n", filenameTh);
				exit(1);
			}
		} else if (strcmp(ensemble, "npt") == 0) { // if ensemble is npt write information (not normalized!) to the output file
			if (fprintf(ioth, "     %2d    %.4lf    %.4lf    %.3lf       %.3lf      %.2lf",	ncycles, nacceptper, nvacceptper, etav * eps_o / naver,	esav * eps_o / naver, volav * sigma_o * sigma_o * sigma_o / naver) == EOF) {
				printf("ERROR: cannot write to '%s' file!\n", filenameTh);
				exit(1);
			}

			for (int i = 0; i < NDIM; ++i) { // write volume information (not normalized!) to the output file
	...
	if (strcmp(ensemble, "nvt") == 0) {  // if ensemble is nvt write information (not normalized!) to the output file
		if (fprintf(iothi, "     %2d    %.4lf        %.3lf       %.3lf\n", ncycles, nacceptper, *etotal * eps_o, esr * eps_o) == EOF) {
			printf("ERROR: cannot write to '%s' file!\n", filenameThi);
			exit(1);
		}
		// and print information (not normalized!)
		printf("     %2d    %.4lf        %.3lf       %.3lf\n", ncycles, nacceptper, *etotal * eps_o, esr * eps_o);
	} else if (strcmp(ensemble, "npt") == 0) { // if ensemble is npt write information (not normalized!) to the output file
		if (fprintf(iothi, "     %2d    %.4lf        %.4lf         %.3lf      %.3lf     %.2lf",	ncycles, nacceptper, nvacceptper, *etotal * eps_o, esr * eps_o, v0 * sigma_o * sigma_o * sigma_o) == EOF) {
			printf("ERROR: cannot write to '%s' file!\n", filenameThi);
			exit(1);
		}
		...
		// and print information (not normalized!)
		printf("     %2d     %.4lf      %.4lf   %.3lf     %.3lf   %.2lf\n", ncycles, nacceptper, nvacceptper, *etotal * eps_o, esr * eps_o, v0 * sigma_o);
	}
...
void initOutputFiles(const char *ensemble, const int nsp, char **atoms) {
	...
	for (int i = 0; i < nsp; ++i) {
		// write to output file the specie's chemical character representation as a header
	...
	if (strcmp(ensemble, "nvt") == 0) { // if ensemble is nvt write the header to output file
	...
	} else if (strcmp(ensemble, "npt") == 0) { // if ensemble is npt write the header to output file
		if (fputs("No. moves  % accept.   % accept. vol.   <E_tot>      <E_sr>    <Vol>      <Lx>     <Ly>    <Lz>\n", ioth) == EOF) {
			printf("ERROR: cannot write to '%s' file!\n", filenameTh);
			exit(1);
		}
		...
		// if ensemble is nvt write the header to output file
		if (fputs("No. moves  % accept.   % acc. vol.    E_tot          E_sr          Vol       Lx        Ly        Lz\n", iothi) == EOF) {
			printf("ERROR: cannot write to '%s' file!\n", filenameThi);
			exit(1);
		}
...
void writeConf(const int natoms, int *nspps, char **atoms, double *r, double *runit) {
	...
	for (int i = 0; i < natoms; ++i) {
		for (int j = 0; j < NDIM; ++j) {
			rr[i * NDIM + j] = r[i * NDIM + j] * runit[j]; // not normalized particles positions (xyz)
	 	}
	}
	...
	if (fprintf(fp, "%d\n Initial Configuration\n", natoms) == EOF) { // write header to ouput file
		printf("ERROR: cannot write to '%s' file!\n", filename);
		exit(1);
	}
	...
	for (int i=0; i < natoms; ++i) {
		if (i < nspps[nsppsc]) {
			if (fprintf(fp,"%s ", atoms[nsppsc]) == EOF) { // write specie's chemical character representation to output file
				printf("ERROR: cannot write to '%s' file!\n", filename);
				exit(1);
			}
		} else {
		...
		for (int j = 0; j < NDIM; ++j) {
			if (fprintf(fp, "%lf ", rr[i * NDIM + j]) == EOF) { // write particle positions (xyz) to output file
				printf("ERROR: cannot write to '%s' file!\n", filename);
	 			exit(1);
			}
		}
